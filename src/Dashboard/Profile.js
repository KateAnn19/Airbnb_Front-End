...
const Profile = ({ userDetails, setUserDetails, isEditing, setIsEditing }) => {
  ...
  const onDelete = (id) => {
    axiosWithAuth()
      .delete(
        `https://kmcgeeka-airbnboptimal.herokuapp.com/listings/delete/${id}`
      )
      .then((res) => {
        console.log(res);
        setUserDetails({
          ...
          <>
            {isEditing ? (
              ...
              <div className="cards profileCard">
                <div className="left">
                  <img
                    src="https://freedesignfile.com/upload/2012/09/Houses-3-.jpg"
                    alt=""
                  />
                  <button onClick={() => onDelete(list.listingid)}>
                    Delete
                  </button>
                  <button
                    className="edit"
